...
export default function Login() {
  ...
  <div className="container px-5 py-24 mx-auto flex flex-wrap items-center">
    ...
    <div className="lg:w-2/6 md:w-1/2 bg-gray-100 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
      ...
      <div className="flex">
      <button 
        className="w-1/2 text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg"
        onClick={(e) => letsLogin(id, email, e)}
      >
        로그인
      </button>
      <button 
        className="w-1/2 text-white bg-indigo-500 ml-4 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg"
        onClick={(e) => logout(e)}
      >
        로그아웃
      </button>
